...
/**
 * Este es la clase Medicina, para este proyecto solo se puede comprar y consultar la lista de
 * medicinas.
 *
 * @author rventurar
 */
public class Medicine extends UnicastRemoteObject implements MedicineInterface {
  ...
  public Medicine getMedicine(int amount) throws Exception {
    if (this.stock <= 0) throw new StockException("Stock empty");

    if (this.stock - amount < 0) throw new StockException("Stock not amount of medicine");
